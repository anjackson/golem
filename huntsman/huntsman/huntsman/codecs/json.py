...
class Encoder(BaseEncoder, CrawlFrontierJSONEncoder):
    ...
    def encode(self, obj):
        ...
        return super(Encoder, self).encode(encoded).encode('utf-8')
...
class Decoder(json.JSONDecoder, BaseDecoder):
    ...
    def decode(self, message):
        message = _convert_from_saved_type(super(Decoder, self).decode(message.decode('utf-8')))
    ...
    def decode_request(self, message):
        obj = _convert_from_saved_type(super(Decoder, self).decode(message.decode('utf-8')))
        return self._request_model(url=obj['url'],
                                   method=obj['method'],
                                   headers=obj['headers'],
                                   cookies=obj['cookies'],
