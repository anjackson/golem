...
class Encoder(BaseEncoder, CrawlFrontierJSONEncoder):
    ...
    def encode(self, obj):
        return super(Encoder, self).encode(obj).encode('utf-8')
...
class Decoder(json.JSONDecoder, BaseDecoder):
    ...
    def decode(self, message):
        message = super(Decoder, self).decode(message.decode('utf-8'))
    ...
    def decode_request(self, message):
        obj = super(Decoder, self).decode(message.decode('utf-8'))
        return self._request_model(url=obj['url'],
                                   method=obj['method'],
                                   headers=obj['headers'],
                                   cookies=obj['cookies'],
